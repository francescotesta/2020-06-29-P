...
public class PremierLeagueDAO {
	...
	public List<Match> matchPerMese(int mese, Map<Integer, Match> idMap) {
		String sql = "SELECT m.MatchID, m.TeamHomeID, m.TeamAwayID, m.teamHomeFormation, m.teamAwayFormation, m.resultOfTeamHome, m.date, t1.Name, t2.Name   "
				+ "FROM Matches m, Teams t1, Teams t2 "
				+ "WHERE m.TeamHomeID = t1.TeamID AND m.TeamAwayID = t2.TeamID "
				+ "AND MONTH(m.date) = ?";
	...
	public List<Adiacenza> getArchi(Map<Integer, Match> idMap, int mese, int min){
		String sql = "SELECT m1.MatchID as id1, m2.MatchID as id2, COUNT(*) as peso "
				+ "FROM Matches m1, Matches m2, Actions a1, Actions a2 "
				+ "Where m1.MatchID = a1.MatchID AND m2.MatchID = a2.MatchID AND m1.MatchID > m2.MatchID "
